function [ output_args ] = matlab2VTK_fluid_lin(nodes,elems,P,filename)
...
% elems: An m×N matrix where each row represents an element by listing the indices of its nodes. The exact number of nodes per element depends on the element type being used (for linear elements, it might be less than 20).
...
% Outputs
% The function generates a .vtk file with the specified filename containing the mesh and pressure data.
...
cell_types = 12*ones(nb_elem,1);                    % | For CELL_TYPES in VTK
